...
@companies_bp.route("/create_company", methods=["POST"])
@login_required
def create_company():
    try:
        ...
        existing_company = db_session.query(Company).filter_by(name=company_name).first()
        if existing_company:
        ...
        traceback.print_exc()
        return jsonify({"error": "Error creating company"}), 500
